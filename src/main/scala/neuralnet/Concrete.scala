...
object Concrete extends App {
    ...
    val concrete = Relation.apply("data/concrete.csv", "concrete", 
                                domain=null, key = 0, eSep = ",", cPos = null)
    concrete.show(5)

    val (x,y) = concrete.toMatriDD(0 to 7, 0)
    val ox = new MatrixD(x.dim1, 8, 1.0) ++^ x // x augmented with vector of ones
    val normX = new MatrixD(ox.normalizeU()) //normalizing features 
    val normY: VectoD = VectorD(y.normalize()) //normalizing response
    val matY: MatrixD = new MatrixD(concrete.toMatriD(Seq(0))) //NormY as matrix
    ...
    */
    def plot_and_save (regMat: MatriD, path: String, basePath: String = "plots/scala/Concrete/")  = { 
